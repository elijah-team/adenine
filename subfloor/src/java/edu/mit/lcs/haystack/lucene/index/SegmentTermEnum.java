...
final class SegmentTermEnum extends TermEnum implements Cloneable {
  ...
  protected Object clone() {
    ...
    if (term != null) clone.growBuffer(term.text().length());  /* HAYSTACK */
  ...
  final void seek(long pointer, int p, Term t, TermInfo ti)
    ...
    prev = null;
    termInfo.set(ti);
    growBuffer(term.text().length());		  /* HAYSTACK */
  }
  ...
  private final Term readTerm() throws IOException {
    ...
    return new Term(fieldInfos.fieldName(input.readVInt()),
            new String(buffer, 0, totalLength));
    /* HAYSTACK: Removed ", false" used by invisible constructor */
    /* Cost is some loss of performance */
  }

  
  /* HAYSTACK: changed .text to .text() */
