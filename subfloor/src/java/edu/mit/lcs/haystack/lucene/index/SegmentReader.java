...
final class SegmentReader extends IndexReader {
	...
	private void initialize(SegmentInfo si) throws IOException {
		...
		/* HAYSTACK */
		try {
			forwardReader = new ForwardReader(directory(), segment);
	...
	final Vector files() throws IOException {
		...
		final String ext[] = new String[] { "cfs", "fnm", "fdx", "fdt", "tii",
				"tis", "frq", "prx", "del", "tvx", "tvd", "tvf", "tvp" };
		...
		/* HAYSTACK */
		// forward index files 
		files.addElement(segment + ".fwd");
		files.addElement(segment + ".fwi");
		files.addElement(segment + ".fwu");
		/* END HAYSTACK */
	 ...
	 *            if true, returns only Indexed fields that have term vector
	 *            info, else only indexed fields without term vector info
	...
	private final void openNorms(Directory cfsDir) throws IOException {
		for (int i = 0; i < fieldInfos.size(); i++) {
			...
			if (fi.isIndexed) {
				...
				Directory d = directory().fileExists(fileName) ? directory()
						: cfsDir;
	...
	final synchronized public void delete(Object uniqueID) throws IOException {
		if (forwardReader != null) {
			int docNum = fieldsReader.getDocumentNumber(uniqueID);
			delete(docNum);
		}
	}
	...
	/**
	 * HAYSTACK ONLY:
	 * returns the forward index reader
	 */
	public ForwardReader getForwardReader() {
		return forwardReader;
	}

	/**
	 * HAYSTACK ONLY: returns the index of the document given a primary
	 * key value.   returns -1 if not found or forward indices not supported.
	 */
	public final synchronized int getPosition(Object uniqueID)
			throws IOException {
		if (forwardReader == null) {
			return -1;
		} else {
			try {
				return fieldsReader.getDocumentNumber(uniqueID);
