 ...
 * 
 * @version $Id: MultiReader.java,v 1.1 2004/07/10 00:41:40 yks Exp $
 */
public class MultiReader extends IndexReader {
    ...
    /* HAYSTACK START */
    public void delete(Object uniqueID) throws IOException {
        for (int x = 0; x < subReaders.length; x++) {
            IndexReader r = subReaders[x];
            if ((r != null)) {
                Document d = r.document(uniqueID);
                if (d != null) {
                    r.delete(uniqueID);
                    break;
                }
    ...
    public int getPosition(Object uniqueID) throws IOException {
        for (int x = 0; x < subReaders.length; x++) {
            IndexReader r = subReaders[x];
            if ((r != null)) {
                int d = r.getPosition(uniqueID);
                if (d != -1) {
                    return d;
                }
            }
    ...
    MultiReader(Directory directory, SegmentInfos sis, boolean closeDirectory,
    ...
    public Document document(int n) throws IOException {
        int i = readerIndex(n); // find segment num
        return subReaders[i].document(n - starts[i]); // dispatch to segment
                                                      // reader
    }
