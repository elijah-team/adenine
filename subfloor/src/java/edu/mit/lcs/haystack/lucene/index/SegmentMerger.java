...
final class SegmentMerger {
	...
	private static final String COMPOUND_EXTENSIONS[] = new String[] { "fnm",
			"frq", "prx", "fdx", "fdt", "tii", "tis" };

	private static final String VECTOR_EXTENSIONS[] = new String[] { "tvx",
			"tvd", "tvf" };
	...
	private final void createCompoundFile() throws IOException {
		...
		ArrayList files = new ArrayList(COMPOUND_EXTENSIONS.length
				+ fieldInfos.size());
		...
		try {
			for (int r = 0; r < readers.size(); r++) {
				...
				for (int docNum = 0; docNum < maxDoc; docNum++) {
					...
					TermFreqVector[] sourceTermVector = reader
	...
	/* HAYSTACK */
	/**
	 * HAYSTACK ONLY: merges all the forward indices
	 * 
	 * @throws IOException
	 */
	private final void mergeForward() throws IOException {
		String primaryField = null;
		for (int i = 0; i < readers.size(); i++) {
			SegmentReader reader = (SegmentReader) readers.elementAt(i);
			ForwardReader fr = reader.getForwardReader();
			if (fr != null)
				primaryField = fr.getPrimaryField();
		}
		...
		try {
			for (int i = 0; i < readers.size(); i++) {
				...
				if (maxDoc != fr.size()) {
					throw new RuntimeException("maxDoc != fr.size()");
				}

				for (int d = 0; d < maxDoc; d++) {
					if (deletedDocs == null || !deletedDocs.get(d)) {
						// skip deleted docs
						FrequencyMap fm = fr.getFrequencyMap(d);
						fw.writeDocument(fm);
					}
				}
			}
