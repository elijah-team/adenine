...
final class DocumentWriter {
    ...
    final void addDocument(String segment, Document doc) throws IOException {
         ...
         * postings[i]; System.out.print(posting.term); System.out.print("
         * freq=" + posting.freq); System.out.print(" pos=");
    ...
    */
    ...
    private final void addPosition(String field, String text, int position) {
        ...
        if (ti != null) { // word seen before
            ...
            //Term term = new Term(field, text, false);  /* HAYSTACK */
    ...
    private static final void quickSort(Posting[] postings, int lo, int hi) {
        ...
        for (;;) {
            ...
            while (left < right
                    && postings[left].term.compareTo(partition) <= 0)
                ++left;
    ...
    private final void writePostings(Posting[] postings, String segment)
        ...
        try {
            ...
            // make an effort to close all streams we can but remember and
            // re-throw
    ...
    // Tokenizes the fields of a document into Postings.
    private final void invertDocument(Document doc, String segment)
            throws IOException {
        ...
        try {
            ...
            while (fields.hasMoreElements()) {
                ...
                if (field.isIndexed()) {
                    ...
                    if (fieldName.equals(primaryField)) {
                        if (primaryFieldValue != null)
                            throw new RuntimeException(
                                    "primaryField has appeared twice!");
                    ...
                    if (!field.isTokenized()) { // un-tokenized field
                        ...
                        try {
                            for (Token t = stream.next(); t != null; t = stream
                                ...
                                if (bForward) {
                                    Integer i = (Integer) hashWords.get(t
                                            .termText());
                                    if (i == null) {
                                        hashWords.put(t.termText(),
                                                new Integer(1));
                                    } else {
                                        hashWords.put(t.termText(),
                    ...
                    fieldLengths[fieldNumber] = length; // save field length
                    fieldPositions[fieldNumber] = position; // save field
                                                            // position
                    fieldBoosts[fieldNumber] *= field.getBoost();
    ...
    public DocumentWriter(boolean bForward, String primaryField, Directory d,
            Analyzer a, int mfl) {
        directory = d;
        analyzer = a;
        maxFieldLength = mfl;
