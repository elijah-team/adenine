...
public class MelatoninAgent extends GenericService {
	final static Category s_logger = Category.getInstance(MelatoninAgent.class);
	...
	public static MelatoninAgent getMelatoninAgent(IRDFContainer source, IServiceAccessor sa) {
		try {
			RDFNode[] datum = source.queryExtract(new Statement[] {
					new Statement(Utilities.generateWildcardResource(1), Constants.s_rdf_type, MelatoninConstants.s_MelatoninAgent),
					new Statement(sa.getResource(), Constants.s_config_hostsService, Utilities.generateWildcardResource(1))
	...
	public boolean cancelJob(Resource jobID) {
		synchronized (m_jobs) {
			// TODO: possibly implement a more efficient means 
	...
	/* (non-Javadoc)
	 * @see edu.mit.lcs.haystack.server.service.GenericService#cleanup()
	 */
